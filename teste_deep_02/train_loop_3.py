...
import lpips
from utils.metrics import compute_all_metrics  # Importa a função de métricas do arquivo metrics.py
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, ((part1, part2), target) in pbar:
            ...
            if datetime.now() - last_fixed_sample_time >= timedelta(minutes=fixeSampleTime):
                salvar_resultados_fixos(generator, fixed_samples, output_dir=fixed_samples_dest, step_tag=f"{epoch}_{epoch * len(dataloader) + i}")
                last_fixed_sample_time = datetime.now()
